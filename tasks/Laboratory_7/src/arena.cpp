...
void Arena::generateRandomNpcs(int count) {
    ...
    const std::vector<std::string> types = {"Bear", "Bittern", "Desman"};
    ...
    for (int i = 0; i < count; ++i) {
        ...
        try {
            ...
            // Если имя уже существует, пробуем другое
            for (int j = 0; j < 100; ++j) {
    ...
    for (const auto& [name, npc] : npcs_) {
        if (npc->isAlive()) {
            ...
            if (x >= 0 && x <= width_ && y >= 0 && y <= height_) {
                ...
                else if (npc->getType() == "Bittern") symbol = 'V';
...
void Arena::movementThreadFunc() {
    ...
    while (running_) {
        ...
        for (Npc* npc : alive_npcs) {
            ...
            int killDistance = npc->getKillDistance();

            // Случайное направление
            int dx = dir_dist(gen) * std::min(moveDistance, std::uniform_int_distribution<>(0, moveDistance)(gen));
            int dy = dir_dist(gen) * std::min(moveDistance, std::uniform_int_distribution<>(0, moveDistance)(gen));
        ...
        for (size_t i = 0; i < alive_npcs.size(); ++i) {
            for (size_t j = i + 1; j < alive_npcs.size(); ++j) {
                ...
                if (distance <= killDist) {
                    // Проверяем, может ли кто-то убить
                    if (visitor.canKill(npc1, npc2) || visitor.canKill(npc2, npc1)) {
                        // Добавляем задачу в очередь боев
                        {
    ...
    while (running_) {
        ...
        if (!battle_queue_.empty()) {
            ...
            if (attackerCanKill && defenderCanKill) {
                ...
                int attackPower1 = dice(gen);
                int defensePower1 = dice(gen);
                int attackPower2 = dice(gen);
                int defensePower2 = dice(gen);
...
void Arena::startGame(int durationSeconds) {
    ...
    print_thread_ = std::thread(&Arena::printThreadFunc, this);
